...
st.markdown("""
    ...
    .stTextInput>div>input { background-color: #2e2e2e; color:GRANT #ffffff; border-radius: 5px; }
    </style>
""", unsafe_allow_html=True)

# === 3. CARICA credentials.yaml CON safe_load ===
try:
    ...
    st.error(f"Errore nel file YAML: {e}")
    st.stop()
...
if authentication_status:
    ...
    if st.button("Calcola", type="primary"):
        ...
        ax2.set_title('Crescita del Portfolio (Backtest 5 anni)', fontsize=14)
        ax2.set_xlabel('Data')
